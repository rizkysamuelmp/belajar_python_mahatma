...
def extract_precise_quote(text):
    """Extract most precise sentence from quote"""
    if pd.isna(text):
        return None
    text = str(text).strip()
...
# Theme 1: Revival Memory - only with valid media names
doc.add_heading('Tema 1: Revival Memory - Representasi Trend Y2K dalam Media Online', 1)

theme1_data = []
for idx, row in df.iterrows():
    if pd.notna(row['Media Name']):
        for col in ['Nostalgic', 'Vintage', 'Retro', 'Revival']:
            if pd.notna(row[col]):
                precise = extract_precise_quote(row[col])
                if precise:
                    theme1_data.append({
                        'Media': str(row['Media Name']),
...
# Theme 2: Gen Z - only with valid media names
doc.add_page_break()
doc.add_heading('Tema 2: Identitas Gen Z - Figur Selebriti Olivia Rodrigo Dalam Trend Y2K', 1)

theme2_data = []
for idx, row in df.iterrows():
    if pd.notna(row['Media Name']) and pd.notna(row['Gen Z']):
        precise = extract_precise_quote(row['Gen Z'])
        if precise:
            theme2_data.append({
                'Media': str(row['Media Name']),
                'Date': str(row['Date']) if pd.notna(row['Date']) else '',
